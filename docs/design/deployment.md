...
- The WIF provider's attribute condition pins `attribute.repository == "populationgenomics/themis-internal"`; OIDC
  ...
  Which SA a token may impersonate is then scoped per SA: deploy to `refs/heads/main` tokens, preview to `pull_request`
  tokens.
- **Write** access (deploy) runs only on `push` to `main`, as the deploy SA — its WIF binding is scoped to
  `refs/heads/main` tokens. PRs get a **read-only** identity (the preview SA, WIF binding scoped to `pull_request`
  tokens) used only to run `pulumi preview` and post it as a comment, informing the single PR-approval gate. No PR job
  can mutate cloud state. The read-only token is dev-scoped (synthetic data); the residual malicious-PR risk is bounded
  by the private repo and trusted membership. Cloud-free validation (lint, type-check, unit tests) also runs on PRs.

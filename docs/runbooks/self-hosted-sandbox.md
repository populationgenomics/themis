...
1. **Set the environment id + SPKI pins** — `pulumi config set themis:anthropicEnvironmentId "$ENV_ID"`; the
   `themis:anthropicSpkiPins` are the SPKI/CA hashes for `api.anthropic.com` (comma-separated, base64 sha256 of
   SubjectPublicKeyInfo — pin the intermediate/CA, not the leaf).

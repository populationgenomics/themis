...
**Status:** Accepted. Defines the manifest's source/rendering model and the quote-durable cite model that the KU layer
(deferred) anchors against. Realized in `schema/litcache/source.tsp` + `schema/litcache/manifest.tsp` and the writer.

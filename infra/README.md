...
Guarded only where loss is unrecoverable or externally bound (`protect`, plus `retain_on_delete` where dropping the
declaration must leave the resource standing):

- The reserved load-balancer IP: DNS points at it.

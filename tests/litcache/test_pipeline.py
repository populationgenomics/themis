...
class _BodyFetcher:
    ...
    async def fetch(
        self,
        article_ids: ids.ArticleIds,  # noqa: ARG002 -- Fetcher protocol arg; this stub serves a fixed blob
        *,
...
# stands in so metadata resolution (Crossref rung) produces a valid PubmedArticle.
...
def test_ingests_a_nonoa_paper_end_to_end(conn: pg8000.dbapi.Connection, gcs_bucket: gcs.Bucket) -> None:
    result = _ingest_via_transport(
        gcs_bucket, conn, _seed(), _licence(), fetchers=_NO_OA, transport=_crossref_transport(_NONOA_CROSSREF)
    )
    ...
    # metadata.pb is the resolved (Crossref → PubmedArticle) record, not a caller input.
    metadata_bytes = gcs_bucket.blob(posixpath.join(paper_dir, 'metadata.pb')).download_as_bytes()
    resolved = pubmed_pb2.PubmedArticle.FromString(metadata_bytes)
    assert resolved.medline_citation.article.article_title.value == 'Synthetic Non-OA Fixture'
...
def test_re_run_is_idempotent_and_skips(conn: pg8000.dbapi.Connection, gcs_bucket: gcs.Bucket) -> None:
    transport = _crossref_transport(_NONOA_CROSSREF)
...
def test_ingests_an_oa_paper_via_the_xml_branch(conn: pg8000.dbapi.Connection, gcs_bucket: gcs.Bucket) -> None:
    ...
    result = _ingest_via_transport(
        ...
        transport=_crossref_transport((_OA / 'crossref.json').read_bytes()),
        file_sources=[],
    )

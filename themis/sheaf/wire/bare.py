...
# This process forks only to exec git. With fork support on, gRPC's fork handlers report every
# fork on stderr, which git relays to the pusher as `remote:` lines.
...
class SyncState:
    ...
    `store` is the repository's descriptor, as `Repository.descriptor` produced it and
    `themis.sheaf.stores.from_descriptor` rebuilds it: where the state lives, never a credential.
    """
    ...
    def to_json(self) -> str:
        ...
        return json.dumps({'generation': self.generation, 'store': self.store})
...
class BareRepo:
    ...
    def sync(self) -> store_mod.Snapshot:
        ...
        state = SyncState(generation=snapshot.generation, store=self.store.descriptor())
    ...
    def _hydrate(self) -> store_mod.Snapshot:
        ...
        try:
            ...
            raise errors.CorruptRepository(f'{self.store.repo} names a pack that is gone: {absent}') from absent
        return snapshot

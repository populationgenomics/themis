...
# The BFF's surface is the method-addressed Connect one: every method is a POST whose body is the
# request message. Spelled once here — the tests below are about the gates, not the paths.
_LIST_PROJECTS = '/api/rpc/themis.workbench.rpc.Workbench/ListProjects'
...
def test_a_relative_path_fails_loud() -> None:
    ...
    with pytest.raises(ValueError, match='must start with'):
        client.post(_LIST_PROJECTS.removeprefix('/'), {})
...
def test_a_permitted_response_is_returned_whatever_its_status() -> None:
    ...
    assert client.post(_LIST_PROJECTS, {}).status_code == 500

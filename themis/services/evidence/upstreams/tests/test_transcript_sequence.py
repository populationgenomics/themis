...
def _fetch(handler: httpx.MockTransport) -> transcript_sequence.TranscriptSequenceResult:
...
def test_the_sequence_is_indexed_by_the_transcripts_own_n_coordinates() -> None:
    ...
    result = _fetch(httpx.MockTransport(lambda _: httpx.Response(200, text=_FASTA)))
...
def test_a_record_ncbi_does_not_hold_is_not_found() -> None:
    ...
    with pytest.raises(errors.UnknownVariantError, match='holds no sequence'):
        _fetch(httpx.MockTransport(lambda _: httpx.Response(400, text='Error: Failed to retrieve sequence')))


def test_a_server_fault_stays_an_http_error() -> None:
    with pytest.raises(httpx.HTTPStatusError):
        _fetch(httpx.MockTransport(lambda _: httpx.Response(502, text='bad gateway')))
...
def test_a_4xx_about_the_client_is_not_read_as_an_absent_record(status: int) -> None:
    ...
    """
    with pytest.raises(httpx.HTTPStatusError):
        _fetch(httpx.MockTransport(lambda _: httpx.Response(status, text='no sequence for you')))

...
transient ``httpx2`` transport errors (connect / read / protocol) — with exponential backoff; every
other status (200, 304, 404, other 4xx) is returned to the caller to interpret. Not a general
client: the refresh job owns the ``httpx2.AsyncClient`` and passes it in.
"""
...
async def request_with_retry(
    client: httpx2.AsyncClient, method: str, url: str, *, headers: dict[str, str] | None = None
) -> httpx2.Response:
    """Issue ``method url`` on ``client``, retrying 429/5xx and transient transport errors.

    A transient ``httpx2.TransportError`` (connect / read / protocol) is retried on the same
    exponential backoff and attempt budget as a 429/5xx status, then re-raised once the budget is
    spent. ``httpx2.HTTPStatusError`` is never raised here — a status is returned, not raised.
    ...
    Raises:
        httpx2.TransportError: If every attempt fails with a transient transport error.
    """

"""The variant interface's env contract: which adapter each selector value builds, fail-loud."""
...
def _normalize(backend: variant_backend.VariantBackend, variant: str) -> variant_pb2.NormalizeResponse:
    return asyncio.run(backend.normalize(variant_pb2.NormalizeRequest(variant=variant, genome_build='GRCh38')))


def test_backend_is_required(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv('THEMIS_VARIANT_BACKEND', raising=False)
    with pytest.raises(SystemExit, match='THEMIS_VARIANT_BACKEND'):
        _from_env()


def test_unknown_backend_is_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv('THEMIS_VARIANT_BACKEND', 'bogus')
...
def test_fixture_seed_is_required(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv('THEMIS_VARIANT_BACKEND', 'fixture')
    monkeypatch.delenv('THEMIS_VARIANT_FIXTURE', raising=False)
    with pytest.raises(SystemExit, match='THEMIS_VARIANT_FIXTURE'):
        _from_env()
...
def test_fixture_selector_answers_what_it_was_seeded_and_nothing_else(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv('THEMIS_VARIANT_BACKEND', 'fixture')
    monkeypatch.setenv(
        'THEMIS_VARIANT_FIXTURE',
        json.dumps({'normalize': {'NM_007294.4:c.5074G>C': {'caid': 'CA028800'}}}),
    )
    backend = _from_env()
    assert _normalize(backend, 'NM_007294.4:c.5074G>C').caid == 'CA028800'
    with pytest.raises(errors.UnknownVariantError):
        _normalize(backend, 'NM_007294.4:c.1A>G')
...
def test_live_selector_builds_the_live_adapter(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv('THEMIS_VARIANT_BACKEND', 'live')
    assert isinstance(_from_env(), variant_backend.LiveBackend)

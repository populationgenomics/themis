...
def agent_exposed_file() -> str:
    """The proto declaring the admission option and the `Caller` enum: the one import every cut leaves unused."""
...
def _admission_option_numbers() -> frozenset[int]:
    """The field numbers of every method option the admission file declares — the marks a cut removes."""
    from themis.rpc import sandbox_options_pb2  # noqa: PLC0415

    return frozenset(ext.number for ext in sandbox_options_pb2.DESCRIPTOR.extensions_by_name.values())
...
def _cuts(
    ...
    mark_numbers: frozenset[int],
) -> list[_Cut]:
    """Every region to delete from `file`'s text: doomed declarations whole, the marks by their exact span.

    A mark is any option on a kept rpc that the admission file declares — who may call an rpc is the
    server's to read, so none of them ships to the guest. A repeated option's locations carry a
    seventh path element, the value's index, and are cut one by one.
    """
    prefix = f'.{file.package}' if file.package else ''
    cuts: list[_Cut] = []
    for location in file.source_code_info.location:
        ...
        if len(path) == 2:
            ...
            (len(path) == 6 or len(path) == 7)  # two `==`, not `in`: pyright narrows the tuple's length on `==`
            and (path[0], path[2], path[4]) == (_SERVICE, _METHOD, _METHOD_OPTIONS)
            and path[5] in mark_numbers
        ):
            # A mark itself, cut to the character: it may share its line with the rpc it marks.
...
def cut_file(
    ...
    text = _apply(lines, _cuts(file, lines, kept_types, kept_methods, needed, _admission_option_numbers()))
